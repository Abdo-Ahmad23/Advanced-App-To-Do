...
class ProjectTaskLine(models.Model):
    ...
    assign_to = fields.Many2one('employee', string='Assigned To', related='task_id.assign_to')
    description = fields.Text(string='Description', related='task_id.description')
    due_date = fields.Date(string='Due Date',related='task_id.due_date')
    # state = fields.Date(string='Due Date',related='task_id.due_date')
    state=fields.Selection(string='State',related='task_id.state')
